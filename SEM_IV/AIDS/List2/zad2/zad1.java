...
public class zad1 {
	...
	public static int[] insertionSort(int[] input, int lenght, boolean fromSmallest) {
		...
		if (fromSmallest) {
			...
			while (j < lenght) {
				value = input[j];
				for (int i = j - 1; i >= 0; i--) {
					System.out.println("Porównujê " + sortedTable[i] + " <= " + value);
					comparCounter++;
		...
		if (lastIndex != firstIndex) {
			...
			if (fromSmallest) {
				...
				while (i < lenght1 && j < lenght2) {
					System.out.println("Porównujê " + sortedTable[i] + " >= " + sortedTable[j]);
					comparCounter++;
	...
	public static int[] quickSort(int[] input, boolean fromSmallest, int firstIndex, int lastIndex) {
		...
		if (fromSmallest) {
			...
			for (int j = firstIndex + 1; j <= lastIndex; j++)
				if (value > result[j]) {
					System.out.println("Porównujê " + value + " > " + result[j]);
					comparCounter++;
					i++;
					swap(result, i, j);
	...
	public static int[] dualPivotSort(int[] input, boolean fromSmallest, int firstIndex, int lastIndex) {
		...
		if (fromSmallest) {
			System.out.println("Porównujê " + result[pivotS] + " > " + result[pivotL]);
			comparCounter++;
			...
			for (int j = firstIndex + 1; j < z; j++) {
				if (counterS > counterL) {
					System.out.println("Porównujê " + valueS + " > " + result[j]);
					comparCounter++;
					if (valueS > result[j]) {
						i++;
						counterS++;
						...
						System.out.println("Porównujê " + valueL + " < " + result[j]);
						comparCounter++;
						z--;
						counterL++;
						swap(result, z, j);
	...
	public static void main(String[] args) {
		...
		System.out.println("Podaj d³ugoœc tablicy");
		...
		System.out.println("Podaj tablicê");
		...
		System.out.println();
		System.out.println("Liczba porównañ : " + comparCounter + 
				" Liczba przestawieñ : " + swapCounter + " Liczba posortowanych elementów : " + length);
	}
